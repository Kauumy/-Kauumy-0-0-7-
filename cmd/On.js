  ...
  try{
    ...
    on.forEach((f) => {
        f.send(msgSend).then(() => {
          ...
          console.log(`Cliente: ${msg.author.tag} | Comando: On | Membro: ${f.tag} | Status: Mensagem enviada | Total enviadas ${countSendO} de ${countMembers}.`);
        }).catch(() => {
          countSendN = countSendN+1;
          console.log(`Cliente: ${msg.author.tag} | Comando: On | Membro: ${f.tag} | Status: Mensagem não enviada | Total não enviadas ${countSendN} de ${countMembers}.`);
        });
    });
  }catch(err){
