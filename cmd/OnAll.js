...
module.exports.run = async (client, msg, args) => {
  ...
  let allA = client.users.filter(m => m.presence.status === 'idle');
  let allD = allA.concat(client.users.filter(m => m.presence.status === 'dnd'));
  let all = allD.concat(client.users.filter(m => m.presence.status === 'online'));
  ...
  try{
    ...
    all.forEach((f) => {
        f.send(msgSend).then(() => {
          ...
          console.log(`Cliente: ${msg.author.tag} | Comando: OnAll | Membro: ${f.tag} | Status: Mensagem enviada | Total enviadas ${countSendO} de ${countMembers}.`);
        }).catch(() => {
          countSendN = countSendN+1;
          console.log(`Cliente: ${msg.author.tag} | Comando: OnAll | Membro: ${f.tag} | Status: Mensagem não enviada | Total não enviadas ${countSendN} de ${countMembers}.`);
        });
    });
  }catch(err){
